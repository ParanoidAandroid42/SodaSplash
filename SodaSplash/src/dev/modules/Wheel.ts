namespace Dev.Modules {
    ...
    export class Wheel extends PIXI.utils.EventEmitter {
        ...
        private initProperties(){
            ...
            let wheelPart = Object.keys(WheelPartSort);
            for(let i = 0; i<wheelPart.length; i++){
                this.setWheelPart(WheelPartSort[wheelPart[i]]);
            }
            
            // this._indicator = new Core.Modules.Sprite2d(322,198,aI.WheelIndicator,this._wheelContainer2d);
            // this._indicator.rotation = 1.22;
            // this._indicator.scale.set(.85,1);

            // this.plane2d.setAxisY(new PIXI.Point(2316,1148),1);
           // this._plane2d.setAxisX(new PIXI.Point(363,-489),1);

            let w = this._frames.frameBase.width;
            let h = this._frames.frameBase.height;
            ...
            //  this._nextIcon = new Core.Modules.Sprite2d(495,176,null,null,aI.WheelNextIcon,this._wheelContainer2d);
             // this._nextIcon.rotation = Math.PI * 0.125;
        }
        ...
        private playWheelsScaleUpAnimation(partIndex:WheelPartSort,endIndex:number){
            ...
            let scale = 1;
            let part : Core.Modules.Sprite2d;

            switch(partIndex){
                case WheelPartSort.Red: part = this._frames.redBase; scale = 1; break;
                case WheelPartSort.Yellow:  part = this._frames.yellowBase; scale = 1.25; break;
                case WheelPartSort.Purple:  part = this._frames.purpleBase;scale = 1.66; break;
                case WheelPartSort.Green:  part = this._frames.greenBase; scale = 2.5; break;
            }
            ...
            switch(partIndex){
                ...
                let scaleInner = 1;
                switch(partIndex){
                    case WheelPartSort.Red: scaleInner = 1; break;
                    case WheelPartSort.Yellow: scaleInner = 1.1; break;
                    case WheelPartSort.Purple: scaleInner = 1.45; break;
                    case WheelPartSort.Green: scaleInner = 1.5; break;
                }
                this.playWheelScaleUp(this._inner,part,partIndex,scaleInner,endIndex);
                case WheelPartSort.Light: this.playWheelScaleUp(this._light,part,partIndex,scale,endIndex);
                break;
            }
        }

        private playWheelPartCircle(part:Core.Modules.Sprite2d,rotation:number,partIndex:number,endIndex:number){
            TweenMax.to(part,1.2,{rotation:part.rotation-rotation,ease:"sine.out",onComplete:()=>{
                this._wheelSpark.visible = true;
                this._wheelSpark.playAnimation(Enum.AnimNames.WheelSparkLoop);
                TweenMax.to(part,2,{rotation:part.rotation + Math.PI*4+Math.PI * 0.125,ease:"power0",onComplete:()=>{
                    this._wheelSpark.playAnimation(Enum.AnimNames.WheelSparkOutre);
                    this._wheelSpark.onComplete = ()=>{this._wheelSpark.visible = false;}
                    TweenMax.to(part,1.2,{rotation:part.rotation + rotation,yoyo:true,ease:"back.out(3)",onComplete:()=>{
        ...
        private playWheelScaleUp(part:Core.Modules.Sprite2d,filterPart:Core.Modules.Sprite2d,partIndex:number,scale:number,endIndex:number){
            TweenMax.to(part.scale,.75,{x:scale,y:scale,delay:1,ease:"back.out(2)",
                onStart:()=>{
                    ...
                    Core.Managers.TickerManager.instance.addTimeout("wait",1,()=>{
            ...
            // TweenMax.fromTo(this._mask.scale,.75,{x:1,y:1},{x:1.1,y:1.1,delay:1,yoyo:true,ease:"back.out(2)",
            // onStart:()=>{
            // }}
            // );
        }
