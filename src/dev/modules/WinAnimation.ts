namespace Dev.Modules {
    export class WinAnimation extends PIXI.utils.EventEmitter {
        ...
        /**
         * Creates an instance of WinAnimation.
        ...
        /**
         * Initializes the properties of the WinAnimation.
         */
        private initProperties() {
            ...
            this._coinContainers = [];
        }

        /**
         * Plays the win animation based on the specified animation type and information.
         * @param animType - The type of win animation.
         * @param winAnimInfo - The win animation information.
        ...
        public playWinAnimation(animType: Enum.WinType, winAnimInfo: Interface.IWinInfo, amount: number) {
            ...
            const coinContainer = new Core.Modules.Container(0, 300, this._backContainer, 'CoinContainer');
        ...
        /**
         * Plays the free spin start animation.
         * @param winAnimInfo - The win animation information.
         * @param amount - The win amount.
         */
        private playFreeSpinStartAnimation(winAnimInfo: Interface.IWinInfo, amount: number) {
            ...
            this._winTimeline.fromTo(
                this._freeSpinFruit.scale,
                winAnimInfo.duration,
                { x: 5, y: 5 },
                { x: 1, y: 1, ease: winAnimInfo.ease },
            );
            this._winTimeline.to(
                this._freeSpinFruit,
                winAnimInfo.duration,
                { alpha: 1, ease: winAnimInfo.ease },
                '-=' + winAnimInfo.duration,
            );
            ...
            this._winTimeline.to(
                this._uiHeader,
                winAnimInfo.duration * 2,
                { y: -500, ease: winAnimInfo.ease },
                '+=' + winAnimInfo.showTime,
            );
            this._winTimeline.to(
                this._winHeader,
                winAnimInfo.duration * 2,
                { y: r.height + 500, ease: winAnimInfo.ease },
                '-=' + winAnimInfo.duration * 2,
            );
            this._winTimeline.to(
                this._winAmount,
                winAnimInfo.duration * 2,
                { alpha: 0, ease: winAnimInfo.ease },
                '-=' + winAnimInfo.duration * 2,
            );
            this._winTimeline.fromTo(
                this._freeSpinFruit.scale,
        ...
        /**
         * Plays the big win animation.
         * @param winAnimInfo - The win animation information.
         * @param amount - The win amount.
         * @param end - The end value for the animation.
         */
        private playBigWinAnimation(winAnimInfo: Interface.IWinInfo, amount: number, end: number) {
            const winAmountTween = TweenMax.fromTo(
                this._winAmount.scale,
                0.25,
                { x: 1, y: 1 },
                { x: 1.2, y: 1.2, ease: 'back.out(0.5)', yoyo: true, repeat: -1 },
            );
            const r = Dev.Config.GameConfig.DisplayConfig;
            ...
            this._winTimeline.to(
                this._winHeader,
                winAnimInfo.duration,
                { y: r.height / 2 + 50, ease: winAnimInfo.ease },
                '+=' + winAnimInfo.showTime,
            );
            ...
            const counter = { const: this._winAmountCount };
            TweenMax.to(counter, winAnimInfo.duration * 5, {
                const: this._winAmountCount + amount,
                onUpdate: () => {
                    this._winAmount.text = Math.ceil(counter.const).toString();
                },
                ease: 'power0',
            });
        ...
        /**
         * Shows the win animation.
         * @param duration - The duration of the animation.
         * @param amount - The win amount.
         */
        public showWinAnimation(duration: number, amount: number) {
            this._winAmountCount = 0;
            TweenMax.fromTo(this._winAmount.scale, 0.25, { x: 1, y: 1 }, { x: 1.2, y: 1.2, ease: 'back.out(0.5)', yoyo: true, repeat: 1 });
            TweenMax.to(this._container, duration, {
                alpha: 0,
                delay: duration,
                onComplete: () => {
                    for (let i = 0; i < this._coinContainers.length; i++) this._coinContainers[i].destroy({ children: true });
                    this.emit(Enum.Listeners.OnAnimationAction, Enum.AnimListener.PlayNextAnimation);
                },
            });
        }
